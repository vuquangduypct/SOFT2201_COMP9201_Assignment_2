...
from quiz_platform.question_bank.rules import rules_from_request
...
def normalise_question(record: dict) -> dict:
    ...
    if isinstance(question.get("estimated_minutes"), str) and question["estimated_minutes"].strip().isdigit():
...
def validate_question(question: dict) -> list[str]:
    ...
    if not isinstance(question.get("estimated_minutes"), int) or question["estimated_minutes"] <= 0:
